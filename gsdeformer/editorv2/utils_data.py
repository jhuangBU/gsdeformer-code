...
def list_cameras_full(source_path: Path) -> Tuple[List[CameraInfo], int, int]:
    eval = True
    ...
    return cameras, len(scene_info.train_cameras), len(scene_info.test_cameras)


def list_cameras(source_path: Path) -> List[CameraInfo]:
    return list_cameras_full(source_path)[0]
