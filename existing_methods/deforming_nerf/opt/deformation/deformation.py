...
def volume_render_image_deformed(grid, cam, cbd, render_orig=False, benchmark=False, batch_size=30000, num_samples=512):
    deform_times = []
    pre_deform = torch.cuda.Event(benchmark)
    post_deform = torch.cuda.Event(benchmark)

    pre_render = torch.cuda.Event(benchmark)
    post_render = torch.cuda.Event(benchmark)
    ...
    for idx, batch_start in enumerate(range(0, cam.height * cam.width, batch_size)):
        ...
        if benchmark:
            deform_times.append(pre_deform.elapsed_time(post_deform))
    ...
    if benchmark:
        render_t = pre_render.elapsed_time(post_render)
        print("DEBUG deform-times", deform_times)
        deform_t = sum(deform_times)
    else:
        render_t = 0
        deform_t = 0

    return im, disp, deform_t, render_t
...
class CBD:
    ...
    def ensure_cage_coordinate(self, batch_size:int=None):
        if not self.cage_coordinate:
            self.cage_coordinate = compute_cage_coordinate(self.cage_target, self.cage_source,
                                                           coord_type=self.coord_type, res=self.res, batch_size=batch_size)
...
def compute_cage_coordinate(cage_deformed, cage_init, coord_type='HC', res=128, batch_size:int=None):
    if coord_type == 'HC':
        coord = Harmonic(cage_deformed, res=res)
        coord.optimize_voxel_hierarchical()
    elif coord_type == 'MVC':
        coord = MeanValue(cage_deformed, res=res, **({'batch_size': batch_size} if batch_size else dict()))
    elif coord_type == 'GC':
        coord = Green(cage_deformed, cage_init, res=res, **({'batch_size': batch_size} if batch_size else dict()))
    else:
