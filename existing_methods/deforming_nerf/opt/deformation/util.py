...
def load_cages(args):
    ...
    # Apply cage scaling if specified (for scene_scale compatibility)
    if hasattr(args, 'cage_scale') and args.cage_scale is not None:
        cage_source.vertices *= args.cage_scale
        cage_target.vertices *= args.cage_scale
        print(f"Applied cage_scale={args.cage_scale} to cage vertices")
