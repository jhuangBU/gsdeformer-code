...
struct POV {
	...
	float* depthBufferCuda = nullptr;
	float* alphaBufferCuda = nullptr;

	bool _interop_failed = false;
	std::vector<char> fallback_bytes;
	float* fallbackSplatBufferCuda = nullptr;
...
public:
	...
	void SetLightingParameters(int H, int W, float* position, float* rotation, float* proj, float fovy, float* frustums);
	bool IsDrawn();
