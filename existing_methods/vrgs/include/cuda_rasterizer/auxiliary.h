...
__forceinline__ __host__ __device__ float ndc2Pix(float v, int S)
{
	return ((v + 1.0) * S - 1.0) * 0.5;
}

__forceinline__ __host__ __device__ float pix2Ndc(float x, int S) {
	return (2.0 * x + 1.0) / S - 1.0;
}
...
__forceinline__ __host__ __device__ float3 transformVec4x3(const float3& p, const float* matrix)
{
	float3 transformed = {
...
__forceinline__ __host__ __device__ float3 transformVec4x3Transpose(const float3& p, const float* matrix)
{
	float3 transformed = {
