...
{
    // Zero out d_out
    CUDA_SAFE_CALL(cudaMemset(d_out, 0, numElems * sizeof(unsigned int)));
    ...
    unsigned int* d_block_sums = d_tmp;
    CUDA_SAFE_CALL(cudaMemset(d_block_sums, 0, sizeof(unsigned int) * grid_sz));
    ...
    {
        unsigned int* d_dummy_blocks_sums = &d_block_sums[grid_sz];
        CUDA_SAFE_CALL(cudaMemset(d_dummy_blocks_sums, 0, sizeof(unsigned int)));
    ...
    {
        unsigned int* d_in_block_sums = &d_block_sums[grid_sz];
        CUDA_SAFE_CALL(cudaMemcpy(d_in_block_sums, d_block_sums, sizeof(unsigned int) * grid_sz, cudaMemcpyDeviceToDevice));
        sum_scan_blelloch(d_block_sums, d_in_block_sums, grid_sz, &d_in_block_sums[grid_sz]);
    }
    
    //// Uncomment to examine block sums
    //unsigned int* h_block_sums = new unsigned int[grid_sz];
    //CUDA_SAFE_CALL(cudaMemcpy(h_block_sums, d_block_sums, sizeof(unsigned int) * grid_sz, cudaMemcpyDeviceToHost));
    //std::cout << "Block sums: ";
