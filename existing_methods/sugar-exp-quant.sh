...
# Usage: ./sugar-exp-quant.sh <scene_name>
# Example: ./sugar-exp-quant.sh quant_mic
...
MODEL_PATH=$project_root/existing_methods/sugar/output/${SCENE}
EVAL_PATH=$project_root/existing_methods/sugar/output/eval_$SCENE

run_python_sugar="conda run -n sugar --no-capture-output python"
...
MODEL_FINE_PATH=$project_root/existing_methods/sugar/output/refined/${SCENE}/sugarfine_3Dgs7000_densityestim02_sdfnorm02_level03_decim1000000_normalconsistency01_gaussperface1
[ ! -d "$MODEL_FINE_PATH" ] && { echo "Error: Refined model not found: $MODEL_FINE_PATH"; exit 1; }

echo "Scene: $SCENE"
echo "==========================================="

# SuGaR loads camera poses from `${MODEL_PATH}/cameras.json` (from GS training output).
...
CONVERT_SCRIPT="$project_root/existing_methods/sugar/convert_transforms_to_cameras_json.py"
...
$run_python_sugar "$CONVERT_SCRIPT" \
...
rm -rf $EVAL_PATH
mkdir -p $EVAL_PATH/test/sugar/renders

$run_python_sugar script_deform_mesh.py \
