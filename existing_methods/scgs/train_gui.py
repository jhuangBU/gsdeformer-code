...
from gaussian_renderer import quaternion_multiply, render, network_gui, render_flow
import sys
...
class GUI:
        ...
        with dpg.window(
            ...
            with dpg.collapsing_header(label="Motion Editing", default_open=True):
                ...
                with dpg.group(horizontal=True):
                    ...
                    def callback_apply_mvc(sender, app_data):
                        # Select all control points
                        if self.animate_tool is None:
                            self.animation_initialize()
                        keypoint_idxs = torch.arange(self.animate_tool.init_pcl.shape[0]).cuda()
                        keypoint_3ds = self.animate_tool.init_pcl[keypoint_idxs]
                        self.deform_keypoints.add_kpts(keypoint_3ds, keypoint_idxs)

                        # Apply MVC transform here (to be implemented by you)
                        src_mesh: Path = self.args.src_mesh
                        dst_mesh: Path = self.args.dst_mesh
                    
                        # This should update the positions of the control points
                        src_mesh = trimesh.load(self.args.src_mesh)
                        dst_mesh = trimesh.load(self.args.dst_mesh)

                        # Convert control points to torch tensor if needed
                        control_points = torch.tensor(self.deform_keypoints.get_kpt(), dtype=torch.float32, device='cuda')
                        ...
                        # Update the keypoint positions with transformed points
                        for i, point in enumerate(all_transformed_points):
                            if i in self.deform_keypoints.keypoints_idx_list:
                                idx = self.deform_keypoints.keypoints_idx_list.index(i)
                                delta = point.cpu().numpy() - self.deform_keypoints.keypoints3d_list[idx]
                                self.deform_keypoints.keypoints3d_delta_list[idx] = delta

                        # Update the displaying gaussians using ARAP deformation
                        with torch.no_grad():
                            if self.deform_mode.startswith("arap"):
                                ...
                                animated_pcl, quat, ani_d_scaling = self.animate_tool.deform_arap(
                                    handle_idx=self.deform_keypoints.get_kpt_idx(), 
                                    handle_pos=self.deform_keypoints.get_deformed_kpt_np(), 
                    ...
                    if self.args.src_mesh and self.args.dst_mesh:
                        dpg.add_button(
                            label="apply_mvc", tag="_button_apply_mvc", callback=callback_apply_mvc
                        )
                        dpg.bind_item_theme("_button_apply_mvc", theme_button)

                    def callback_save_deformed(sender, app_data):
                        # Create a new Gaussian model instance to store deformed state
                        deformed_model = GaussianModel.build_from(
                            self.gaussians, 
                            sh_degree=self.gaussians.max_sh_degree,
                            fea_dim=self.gaussians.fea_dim,
                            with_motion_mask=self.gaussians.with_motion_mask
                        )
                        ...
                        with torch.no_grad():
                            fid = torch.remainder(
                                torch.tensor((time.time() - self.t0) * self.fps_of_fid).float().cuda() / len(
                            ...
                            d_values = self.deform.step(
                                ...
                                animation_d_values=self.motion_animation_d_values
                            )
                            gaussians = self.gaussians
                            d_xyz, d_rotation, d_scaling, d_opacity, d_color = d_values['d_xyz'], d_values[
                                'd_rotation'], d_values[
                                'd_scaling'], d_values['d_opacity'], d_values['d_color']
                            d_rotation_bias = d_values[
                                'd_rotation_bias'] if 'd_rotation_bias' in d_values.keys() else None
                            ...
                            sh_features = torch.cat([pc.get_features[:, :1] + d_color[:, None], pc.get_features[:, 1:]],
                                                    dim=1) if d_color is not None and type(
                                d_color) is not float else pc.get_features
                            shs = sh_features
                            ...
                            assert torch.allclose(shs,
                                                  pc.get_features)  # this is our assumption for copying straight from self.gaussians
                            deformed_model._features_dc = nn.Parameter(self.gaussians._features_dc)
                            deformed_model._features_rest = nn.Parameter(self.gaussians._features_rest)

                        # Save the deformed model
                        output_path = f'./deformed_gaussians_{self.animation_time:.3f}.ply'
                        deformed_model.save_ply(output_path)
                        print(f'Saved deformed Gaussian model to {output_path}')
        
                    if self.args.src_mesh and self.args.dst_mesh:
                        dpg.add_button(
                            label="save_deformed", tag="_button_save_deformed", callback=callback_save_deformed
                        )
                        dpg.bind_item_theme("_button_save_deformed", theme_button)
    ...
    def test_step(self, specified_cam=None):
        ...
        # Print parameter info before render() call
        # print("viewpoint_camera:", cur_cam)
        # print("pc:", gaussians)
        # print("pipe:", self.pipe) 
        # print("bg_color:", self.background.shape if isinstance(self.background, torch.Tensor) else self.background)
        # print("d_xyz:", d_xyz.shape if isinstance(d_xyz, (torch.Tensor, np.ndarray)) else d_xyz)
        # print("d_rotation:", d_rotation.shape if isinstance(d_rotation, (torch.Tensor, np.ndarray)) else d_rotation)
        # print("d_scaling:", d_scaling.shape if isinstance(d_scaling, (torch.Tensor, np.ndarray)) else d_scaling)
        # print("render_motion:", render_motion)
        # print("d_opacity:", d_opacity.shape if isinstance(d_opacity, (torch.Tensor, np.ndarray)) else d_opacity)
        # print("d_color:", d_color.shape if isinstance(d_color, (torch.Tensor, np.ndarray)) else d_color)
        # print("d_rot_as_res:", self.deform.d_rot_as_res)
        # print("scale_const:", vis_scale_const)
        # print("d_rotation_bias:", d_rotation_bias.shape if isinstance(d_rotation_bias, (torch.Tensor, np.ndarray)) else d_rotation_bias)

        out = render(
            viewpoint_camera=cur_cam, 
            pc=gaussians, pipe=self.pipe,
            bg_color=self.background, 
            d_xyz=d_xyz, d_rotation=d_rotation, d_scaling=d_scaling,
            render_motion=render_motion, 
            d_opacity=d_opacity, d_color=d_color,
            d_rot_as_res=self.deform.d_rot_as_res, scale_const=vis_scale_const, d_rotation_bias=d_rotation_bias
        )
