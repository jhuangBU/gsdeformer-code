...
class GaussianModel:
    ...
    def construct_list_of_attributes(self, save_fea=True):
        l = ['x', 'y', 'z', 'nx', 'ny', 'nz']
    ...
    def save_ply(self, path, save_fea=True):
        ...
        dtype_full = [(attribute, 'f4') for attribute in self.construct_list_of_attributes(save_fea)]

        elements = np.empty(xyz.shape[0], dtype=dtype_full)
        attributes = np.concatenate((xyz, normals, f_dc, f_rest, opacities, scale, rotation), axis=1)
        if save_fea and self.fea_dim > 0:
